...
func reequipEquipmentAfterLogin(user *models.User, equipment *models.Equipment, zapLogger *zap.Logger) error {
    ...
    // 更新用户属性
    user.BaseAttributes = toJSON(attrMgr.BaseAttrs)
    user.CombatAttributes = toJSON(attrMgr.CombatAttrs)
    user.CombatResistance = toJSON(attrMgr.CombatRes)
    user.SpecialAttributes = toJSON(attrMgr.SpecialAttrs)
